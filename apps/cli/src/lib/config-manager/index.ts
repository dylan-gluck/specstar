...
import { HOOKS_TEMPLATE } from "./templates/hooks-template";
...
export class ConfigManager {
  ...
  async init(projectPath: string, options?: InitOptions): Promise<void> {
    ...
    try {
      ...
      if (existingConfig && !options?.force) {
        throw new Error(
          `.specstar directory already exists at ${targetDir}. Use --force to overwrite.`,
        );
      }
      ...
      // Create hooks.ts using the embedded comprehensive template
      await Bun.write(hooksPath, HOOKS_TEMPLATE);
      ...
      if (options?.updateClaudeSettings !== false) {
        await this.updateClaudeCodeSettings(
          projectPath,
          hooksPath,
  ...
  async load(): Promise<SpecstarConfig> {
    try {
      ...
      if (!(await file.exists())) {
        throw new Error(
          `Configuration file not found at ${this.configPath}. Run 'specstar --init' to initialize.`,
        );
      }
  ...
  async updateClaudeCodeSettings(
    ...
    try {
      ...
      console.log(
        `   Using hooks script: $CLAUDE_PROJECT_DIR/.specstar/hooks.ts`,
      );
    } catch (error) {
      // Don't fail initialization if Claude settings update fails
      console.warn("⚠️  Could not update Claude Code settings:", error);
      console.warn(
        "   You can manually add the hooks to .claude/settings.json in your project.",
      );
    }
  }
